...
class test_DatabaseHandlerBackend(BaseTestCase):
    ...
    def test_register(self):
        ...
        self.assertEqual(1, Handler.objects.filter(hash='%s.%s_object%s' % (
            PostTransitionHandler.__module__, PostTransitionHandler.__name__, self.objects[1].pk)).count())
        self.assertTrue('%s.%s_object%s' % (
    ...
    def test_register_in_multiprocessing(self):
        ...
        def worker2(q):
            ...
            handlers = second_handler_backend.get_handlers(PostTransitionHandler, self.objects[1])
    ...
    def test_get_handlers_in_multiprocessing(self):
        ...
        Handler.objects.update_or_create(
            hash='%s.%s_object%s' % (PostTransitionHandler.__module__, PostTransitionHandler.__name__, self.objects[1].pk),
            defaults={
