...
class TransitionHandler(Handler):
    ...
    def get_hash(cls, workflow_object, field, source_state=None, destination_state=None, *args, **kwargs):
        return super(TransitionHandler, cls).get_hash(workflow_object, field) + 'source_state' + (str(source_state.pk) if source_state else '') + 'destination_state' + (
            str(destination_state.pk) if destination_state else '')
